...
# 🎯 [버전 컨트롤 타워]
APP_VERSION = "Ver 30.5 (Final Hardcoded)"
...
<style>
    ...
    .report-page h3 { font-size: 20px !important; margin-top: 15px !important; margin-bottom: 8px !important; border-bottom: 2px solid #1A237E; padding-bottom: 8px; color: #1A237E !important; font-weight: 900 !important; width: 100%; display: block; }
...
try:
    ...
    # 모델명 오류 방지를 위해 변수 처리하거나 명시적 선언
    model = genai.GenerativeModel('gemini-2.5-pro') # 2.5-pro는 현재 지원 확인
except Exception as e:
    st.error(f"🚨 AI 서비스 연결 실패: {e}")
    st.stop()  # ◀ 실패 시 다음 로직(NameError 발생 구간)으로 넘어가지 않고 앱 실행을 확실히 중단시킵니다.
...
if btn_single:
    if not u_name.strip(): 
        ...
        with st.spinner(spinner_msg):
            try: # ◀◀ [수정 포인트: try 블록 추가. 이후 공통 연산부터 모두 한 단계 들여쓰기]
                # [공통 연산 - 신청인]
                ...
                st.stop() # 여기서 앱이 확실히 멈추므로 아래 코드가 꼬이지 않습니다.
...
"""
            ...
            # ------------------------------------------------------------------
                ...
                try:
                    ...
                    import re
                    import datetime as dt_mod # ◀ 이미 있다면 중복 방지
                    b3 = chr(96) + chr(96) + chr(96)
                    pattern_str = b3 + "html|" + b3
                    m_ai_content = re.sub(pattern_str, "", m_ai_content).strip()
                    f_ai_content = re.sub(pattern_str, "", f_ai_content).strip()
...
<div style='border: 4px solid #1A237E; padding: 50px 30px; border-radius: 20px; text-align: center; background: white; width: 85%; max-width: 750px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); margin:auto;'>
<div style='border-bottom:4px double #1A237E; padding-bottom:20px; margin-bottom:40px;'><h1 style='font-size: 32px; color: #1A237E; font-weight: 900; margin:0; font-family:\"Malgun Gothic\", sans-serif;'>💞 초연 시공명리 궁합풀이</h1>
</div><div style='background:#F8F9FA; border: 1px solid #E8EAF6; padding: 30px 20px; border-radius: 15px;'>
<h2 style='font-size: 24px; font-weight: 900; color: #1A237E; margin-bottom: 10px; font-family:\"Malgun Gothic\", sans-serif;'>♂️ 남성 : {m_name_target} 님</h2>
<p style='margin-bottom: 25px; color: #555; font-weight: 600; white-space: nowrap;'>[양력] {m_ctx.get('birth_solar', sol_str)} | [음력] {m_ctx.get('birth_lunar', lun_str)}</p>
<h2 style='font-size: 24px; font-weight: 900; color: #D50000; margin-bottom: 10px; font-family:\"Malgun Gothic\", sans-serif;'>♀️ 여성 : {f_name_target} 님</h2>
<p style='margin-bottom: 0; color: #555; font-weight: 600; white-space: nowrap;'>[양력] {f_ctx.get('birth_solar', sol_str)} | [음력] {f_ctx.get('birth_lunar', lun_str)}</p></div>
<p style='font-size: 18px; margin-top: 50px; font-weight: bold;'>{today_str}</p>
<p style='font-size: 22px; font-weight: 900; color: #1A237E; margin-top: 20px; font-family:\"Malgun Gothic\", sans-serif;'>초연 시공명리 연구소</p></div></div>"

                    m_title_html = f"<div style='text-align:center; font-family:\"Malgun Gothic\", sans-serif; margin-bottom:15px; line-height:1.5;'>
<span style='font-size:18px; font-weight:900; color:#1A237E; white-space:nowrap;'>♂️ {m_name_target}님 (남성, {m_marital_target})</span></div>"
                    f_title_html = f"<div style='text-align:center; font-family:\"Malgun Gothic\", sans-serif; margin-bottom:15px; line-height:1.5;'>
<span style='font-size:18px; font-weight:900; color:#D50000; white-space:nowrap;'>♀️ {f_name_target}님 (여성, {f_marital_target})</span></div>"

                    tables_html = "<div class='report-page' style='padding:40px; background:#fff;'>
<div class='vip-inset-frame' style='border:2px solid #1A237E; border-radius:15px; padding:30px;'>"
                    tables_html += f"<div style='text-align:center; border-bottom:4px double #3E2723; padding-bottom:15px; margin-bottom:25px;'>
<h1 style='margin:0; color:#3E2723; font-weight: 900; font-family:\"Malgun Gothic\", sans-serif;'>🗝️ 두 사람의 사주팔자</h1></div>"
                    ...
                    # [수정 적용] 남성/여성 조건에 따른 명확한 순/역행(order) 계산
                    if gender_val == '남성':
                        # 남성은 양년생(0,2,4,6,8)이면 순행, 음년생이면 역행
                        order = 1 if (gan_list.index(ys) % 2 == 0) else -1   
                    else: # 여성
                        # 여성은 양년생(0,2,4,6,8)이면 역행, 음년생이면 순행
                        order = -1 if (gan_list.index(ys) % 2 == 0) else 1
