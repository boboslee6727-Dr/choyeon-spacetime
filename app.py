...
"""
    ...
    """
                ...
                # ------------------------------------------------------------------
                    ...
                    # [범용 데이터 호출]
                    gans, jjis = get_current_saju_data()
                    
                    # 만약 여전히 '?'가 섞여있다면 안전하게 강제 할당
                    if '?' in gans or '?' in jjis:
                        st.warning("사주 데이터를 다시 불러오고 있습니다...")
                        # 여기서 역산 로직 함수를 직접 호출하여 값을 갱신
