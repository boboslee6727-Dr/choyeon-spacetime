...
<tr><td class='header-cell-main' style='border:1px solid #444; background:#f5f5f5; font-weight:900; font-size:14px;'>천간합충</td>{"".join([f"<td style='border:1px solid #444;'>{get_gan_rel_all(i, gans)}</td>" for i in range(4)])}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; background:#f5f5f5; font-weight:900; font-size:14px;'>천간십성</td><td style='border:1px solid #444;'>{get_ss(ds,hs)}</td><td style='border:1px solid #444;'><span style='color:#D50000; font-weight:900;'>日元</span></td><td style='border:1px solid #444;'>{get_ss(ds,ms)}</td><td style='border:1px solid #444;'>{get_ss(ds,ys)}</td></tr>
<tr><td class='header-cell-main' style='border:1px solid #444; background:#E8EAF6; color:#1A237E; font-weight:900; font-size:14px;'>천간</td>{td(hs)}{td(ds)}{td(ms)}{td(ys)}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; background:#E8EAF6; color:#1A237E; font-weight:900; font-size:14px;'>지지</td>{td(hb)}{td(db)}{td(mb)}{td(yb)}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; background:#f5f5f5; font-weight:900; font-size:14px;'>지지십성</td><td style='border:1px solid #444;'>{get_ss(ds,hb)}</td><td style='border:1px solid #444;'>{get_ss(ds,db)}</td><td style='border:1px solid #444;'>{get_ss(ds,mb)}</td><td style='border:1px solid #444;'>{get_ss(ds,yb)}</td></tr>
<tr><td class='header-cell-main' style='padding:0; border:1px solid #444; background:#f5f5f5; font-weight:900; font-size:14px;'>지장간</td>{"".join([f"<td style='padding:0; border:1px solid #444;'>{get_jijanggan_full(ds, jjis[i])}</td>" for i in range(4)])}</tr>
{ji_rel_rows}
<tr><td class='header-cell-main' style='border:1px solid #444 !important; background:#f5f5f5; font-weight:900; font-size:14px;'>십이운성</td>{"".join([f"<td style='color:#0D47A1; border:1px solid #444 !important;'>{get_unsung(ds, jjis[i])}</td>" for i in range(4)])}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444 !important; background:#f5f5f5; font-weight:900; font-size:14px;'>십이신살</td>{"".join([f"<td style='color:#C62828; border:1px solid #444 !important;'>{get_12_shinsal(yb, jjis[i])}</td>" for i in range(4)])}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444 !important; background:#f5f5f5; font-weight:900; font-size:14px;'>일반신살</td>{"".join([f"<td style='vertical-align:top; padding:2px; border:1px solid #444 !important;'>{'<br>'.join(get_general_shinsal_filtered(i, gans, jjis, u_gender)) if get_general_shinsal_filtered(i, gans, jjis, u_gender) else '-'}</td>" for i in range(4)])}</tr>
</table>
"""
                ...
                ji_rel_rows = ""
                for l_idx, r_idx in enumerate([1, 2, 0, 3]):
                    cells = "".join([f"<td style='border:1px solid #444; font-weight:900;'>{get_ji_rel_set(jjis[r_idx], jjis[ci]) if ci!=r_idx else '←('+jjis[r_idx]+')→'}</td>" for ci in range(4)])
                    lbl = f"<td rowspan='4' class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>합충형파해</td>" if l_idx==0 else ""
...
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>천간합충</td>{"".join([f"<td style='border:1px solid #444;'>{get_gan_rel_all(i, gans)}</td>" for i in range(4)])}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>천간십성</td><td style='border:1px solid #444;'>{get_ss(ds,hs)}</td><td style='border:1px solid #444;'><span style='color:#D50000; font-weight:900;'>日元</span></td><td style='border:1px solid #444;'>{get_ss(ds,ms)}</td><td style='border:1px solid #444;'>{get_ss(ds,ys)}</td></tr>
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>천간</td>{td(hs)}{td(ds)}{td(ms)}{td(ys)}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>지지</td>{td(hb)}{td(db)}{td(mb)}{td(yb)}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>지지십성</td><td style='border:1px solid #444;'>{get_ss(ds,hb)}</td><td style='border:1px solid #444;'>{get_ss(ds,db)}</td><td style='border:1px solid #444;'>{get_ss(ds,mb)}</td><td style='border:1px solid #444;'>{get_ss(ds,yb)}</td></tr>
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>지장간</td>{"".join([f"<td style='border:1px solid #444;'>{get_jijanggan_full(ds, jjis[i])}</td>" for i in range(4)])}</tr>
{ji_rel_rows}
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>십이운성</td>{"".join([f"<td style='border:1px solid #444;'>{get_unsung(ds, jjis[i])}</td>" for i in range(4)])}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>십이신살</td>{"".join([f"<td style='border:1px solid #444;'>{get_12_shinsal(yb, jjis[i])}</td>" for i in range(4)])}</tr>
<tr><td class='header-cell-main' style='border:1px solid #444; font-weight:900; font-size:14px;'>일반신살</td>{"".join([f"<td style='border:1px solid #444;'>{'<br>'.join(get_general_shinsal_filtered(i, gans, jjis, u_gender))}</td>" for i in range(4)])}</tr>
</table>
"""
...
"""
                ...
                db_header = (
                    ...
                    # 🚨 옥의 티 제거: 중복 출력을 유발하던 텍스트 정보들을 프롬프트에서 삭제
...
"""
            ...
            # ------------------------------------------------------------------
                ...
                try:
                    ...
                    # 🚨 [문법 오류 및 코드 노출 방어] 아스키코드 암호화 정규식
                    import re
                    b3 = chr(96) + chr(96) + chr(96)
                    pattern_str = b3 + "html|" + b3
                    ...
                    # ---------------------------------------------------------
                    # 🌟 [고도화 1단계] 궁합 표지(Cover Page) 이모지 고정 부활
                    # ---------------------------------------------------------
                    ...
                    # 남성 구역 (빈 타이틀을 넘기고 위에 직접 조립한 타이틀을 얹음)
                    ...
                    # ---------------------------------------------------------
                    # 🌟 [고도화 3단계] 커플 인생 기상도(상하 대운표) 자체 생성 및 치환
                    # ---------------------------------------------------------
                    def build_daeun_row_html(ctx, gender_val):
                        gan_list, ji_list = "甲乙丙丁戊己庚辛壬癸", "子丑寅卯辰巳午未申酉戌亥"
                        order = 1 if (gan_list.index(ctx['yc']) % 2 == 0) == (gender_val == '남성') else -1
                        ...
                        for i in range(10):
                            val = int(ctx['daewun_num']) + i*10
                            c = gan_list[(gan_list.index(ctx['mc'])+(i+1)*order)%10]
                            j = ji_list[(ji_list.index(ctx['mj'])+(i+1)*order)%12]
                            active = "background:#FFEBEE; outline:3px solid #D50000; z-index:2; position:relative;" if (val <= ctx['age'] < val+10) else ""
                    ...
                    m_daeun_html = f"<div style='font-family:\"Malgun Gothic\", sans-serif; font-weight:900; color:#1A237E; margin-bottom:5px; font-size:15px;'>♂️ {m_name_target}님의 대운 흐름</div>" + build_daeun_row_html(m_ctx, '남성')
                    f_daeun_html = f"<div style='font-family:\"Malgun Gothic\", sans-serif; font-weight:900; color:#D50000; margin-top:15px; margin-bottom:5px; font-size:15px;'>♀️ {f_name_target}님의 대운 흐름</div>" + build_daeun_row_html(f_ctx, '여성')
                    ...
                    # 본문 마커를 대운표로 치환 (마커 분실 시 최상단 강제 결합)
