...
(※ 🚨AI 절대 지시: 격국({gyukgook_detail})의 명칭과 그 의미를 가장 먼저 상세히 풀이하십시오. 일간('{ds}')을 뼈대로 격국의 특성이 7궁위 무대에서 어떤 에너지 규모로 발현되는지 현실적으로 조언하십시오.)

<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>2) 내 삶의 리듬과 에너지 균형</span>
(※ 🚨AI 전술 지시: 사주팔자 오행 분포, 조후, 억부의 균형을 분석하고 이를 극복하기 위해 일상에서 추구해야 할 에너지를 에세이로 작성하십시오.)

<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>3) 내 삶의 역동성과 상호작용</span>
(※ 🚨AI 전술 지시: 1. 합충파해 연쇄반응 추적, 2. 묘고(墓庫)의 개고/입고 타이밍, 3. 격각(隔角)으로 인한 이동과 고독을 드라마틱하게 서술하십시오.)

<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>4) 내 삶의 숨겨진 강점과 잠재적 에너지</span>
(※ 🚨AI 전술 지시: 12신살({s12_str}), 일반신살({shinsal_str}), 삼재({cur_samjae})를 앵커로 삼아 현대 심리상담 관점의 강점으로 재해석하십시오.)
</div>
...
2. [일지 지장간 좌법(座法) 해부]: 
   - 규칙: 일지 지장간을 반드시 '초기-중기-여기' 순서로 나열하십시오.
   - 형식: ▪ [지장간 성분] (십성명 + 십이운성 + '좌' + 십이운성 + '궁'): "압축적 핵심 키워드" + "현대적 구어체 비유".
   - [🚨대중 언어 규칙]: '좌/궁'은 (괄호)로 표기하고, 바로 뒤에 "이는 기운이 시작되어 뻗어 나가는 힘을 의미하며, 삶에서는 ~한 방식으로 나타납니다"라는 비유적 풀이를 반드시 추가하십시오.
3. [7궁위 입체 분석]: 7궁위(년/월/일/시)의 관계를 융합하여 육친적/심리적/사회적 관계를 입체적으로 분석하십시오.
   
<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>2) 감추어진 내 속마음</span>
1. [인종법(引從法)의 무의식 타격]: 원국에 없는 결핍 십성을 일지로 인종하여 포태법으로 산출하십시오.
   - 형식: ▪ [십성명] (십성명 + 십이운성 + '종' + 십이운성 + '궁'): "압축적 핵심 키워드" + "현대적 구어체 비유".
   - [🚨대중 언어 규칙]: '종/궁'은 (괄호)로 표기하고, 바로 뒤에 "이는 내면 깊은 곳에서 꿈틀대는 에너지의 흐름을 의미하며, 실제로는 ~한 심리로 작용합니다"라는 비유적 풀이를 반드시 추가하십시오.
2. [공망({gongmang_actual})의 현실 타격]: 공망 궁위 팩트만으로 심리적 공허함과 환경적 한계 조건을 기술하십시오.
...
1. [표면적 기질]: 일간과 원국에 드러난 십성, 12운성, 신살의 상호작용으로 드러난 페르소나 풀이.
2. [일지 지장간 좌법(座法) 해부]: 
   - 규칙: 일지 지장간을 '초기-중기-여기' 순서로 나열하고, 각 성분이 일간과 맺는 관계를 십이운성으로 산출하십시오.
   - 형식: ▪ [지장간 성분] (십성명 + 십이운성 + '좌' + 십이운성 + '궁'): "압축적 핵심 키워드" + "현대적 구어체 비유".
   - [🚨대중 언어 규칙]: '좌/궁'은 (괄호)로 표기하고, 바로 뒤에 "이는 기운이 시작되어 뻗어 나가는 힘을 의미하며, 삶에서는 ~한 방식으로 나타납니다"라는 비유적 풀이를 반드시 추가하십시오.
