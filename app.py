...
with st.sidebar:
    ...
    if u_product == "개인사주":
        ...
        p_y = st.number_input("년", 1900, 2050, value=2010, key="p_y_in")
        p_m = st.number_input("월", 1, 12, value=1, key="p_m_in")
...
if st.session_state.get('app_running', False) and st.session_state.get('run_waterfall', False) and 'global_gans' in st.session_state:
    ...
    if not st.session_state.get('saved_report_iljin'):
        ...
        if gj_str:
            ...
            iljin_prompt = f"""
당신은 명리심리상담사 1급 자격을 갖춘 '초연 박사'입니다.
아래 제공된 [일진 연산 팩트]를 바탕으로, 오늘 하루의 흐름을 대운/세운 분석처럼 깊이 있고 디테일한 '폭포수 에세이'로 작성하십시오.

[내담자 및 환경 팩트]
- 내담자 일주: {m_ilgan}{m_ilji}
- 일진(오늘) 날짜: {t_date.year}년 {t_date.month}월 {t_date.day}일 ({target_year}년 {target_wol}월 {target_il}일)

🚨 [파이썬 연산 시공간 파동 및 체용(무대/사건) 팩트 - 절대 준수]
- 천간 파동(전반기 영향): {gan_res_html}
- 지지 파동(후반기 영향): {r_res_html}
- ☀️ 전반부 체용 구조: 체운({m_che_first}) / 용운({am_yong})
- 🌙 후반부 체용 구조: 체운({m_che_second}) / 용운({pm_yong})
- ✨ 오늘의 핵심 에너지: 십이운성[{day_wunseong}] / 특수기운[{day_shinsal}]

🚨 [AI 통제 헌법 및 출력 규칙]
1. 서론/인사말 철저히 금지. 제공된 템플릿의 첫 줄부터 즉각 출력하십시오.
2. 마크다운 표 생성 및 `**` 강조 기호 사용을 절대 금지합니다. 오직 허락된 HTML 태그만 사용하십시오.
3. 문단과 문단 사이의 빈 줄(Enter) 생성을 금지합니다. 줄바꿈은 `<br>` 태그 하나만 사용하십시오.
4. 단순 길흉 판단을 넘어, [체운+용운]의 결합이 내담자의 실제 현실(직장, 대인관계, 심리상태, 금전)에서 어떤 구체적인 물상으로 일어날지 폭포수처럼 쏟아내어 통변하십시오.

[출력 템플릿] (아래 HTML 태그와 양식을 100% 똑같이 복사하여 출력할 것)
<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>▶ 오늘의 전반부 흐름 (자시~오시, 00:30~13:29)</span>
<div style='padding-left: 20px; margin-top: 5px;'>
    <div style='margin-bottom: 5px;'><b>1) 전통 명리 풀이:</b> (천간 파동 현황과 십이운성을 엮어 구체적 명리 물상으로 상세히 작성)</div>
    <div><b>2) 시공 명리 풀이:</b> (※ 🚨 전반부 체용 구조[{m_che_first} + {am_yong}]를 뼈대로 삼아, 오전 중에 발생할 구체적인 사건과 심리 변화를 폭포수처럼 상세 통변)</div>
</div>
<br>
<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>▶ 오늘의 후반부 흐름 (미시~야자시, 13:30~익일 00:29)</span>
<div style='padding-left: 20px; margin-top: 5px;'>
    <div style='margin-bottom: 5px;'><b>1) 전통 명리 풀이:</b> (지지 형충파해 현황과 특수기운을 엮어 구체적 명리 물상으로 상세히 작성)</div>
    <div><b>2) 시공 명리 풀이:</b> (※ 🚨 후반부 체용 구조[{m_che_second} + {pm_yong}]를 뼈대로 삼아, 오후와 저녁에 발생할 구체적인 사건의 결말과 실질적 행동 지침을 폭포수처럼 상세 통변)</div>
</div>
<br>
<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>✨ 오늘 하루의 개운(開運) 처방</span>
<div style='padding-left: 20px; margin-top: 5px;'>
(오늘의 흉을 피하고 길을 취하기 위한 명리학적이고 실질적인 행동 지침, 방향, 마음가짐 등을 3문장 이상 상세히 조언)
</div>
"""
