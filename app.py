...
import ephem
import google.generativeai as gimport streamlit as st
import pytz
...
"""
   ...
   🚨 [초연 시공명리 특화: 합충형해파(合沖刑害破) 및 행운(行運) 동기화 절대 규칙 - 엄격 준수]
    ...
    (※ AI 지시 🚨[대운 묶음 서술 및 표 중복 출력 절대 금지]: 위 소제목('▷ 지나온 과거 각 대운 분석')을 화면에 반드시 그대로 출력한 후 서술을 시작하십시오. 절대 두 개 이상의 대운을 하나로 묶어서(예: 3세, 13세 통합) 서술하지 마십시오! 반드시 줄을 바꾸어 각각의 대운을 10년 단위로 개별 통변하십시오. 항목 제목은 **• {calc_d}세({간지}대운):** 과 같이 마크다운 굵은 글씨로 강조하십시오. 🚨중요: 표를 생성하는 [DAEWUN_TABLE_HERE] 마커는 이미 상단에 있으므로, 분석 하단에 마커를 절대 중복해서 다시 출력하지 마십시오.)
    ...
    {past_months_html}
    (※ AI 지시 🚨[월운 환각 절대 금지]: 위 소제목을 반드시 화면에 출력하십시오. 올해 지나온 각 과거 월운들을 하나씩 나열하되, 항목의 제목은 반드시 **• 2월(해당간지월):** 과 같이 마크다운 굵은 글씨로 강조하여 2~3줄씩 요약하십시오. 🚨단, 양력 1월은 작년도 세운의 기운이 마무리되는 흐름으로 풀이하십시오. 🚨중요: [WOLWUN_TABLE_HERE] 마커는 이미 상단에 있으니 분석 하단에 절대 중복 출력하지 마십시오.)
