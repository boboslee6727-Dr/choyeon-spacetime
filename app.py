...
with st.sidebar:
    ...
    with st.expander("🔍 신청인 사주간지 역산", expanded=False):
        ...
        with col_g1: ry = st.text_input("년주", value="", key="u_ry", on_change=stop_ai)
        with col_g2: rm = st.text_input("월주", value="", key="u_rm", on_change=stop_ai)
        col_g3, col_g4 = st.columns(2)
        with col_g3: rd = st.text_input("일주", value="", key="u_rd", on_change=stop_ai)
        with col_g4: rt = st.text_input("시주", value="", key="u_rt", on_change=stop_ai)
        
        if st.button("🔍 신청인 생년월일 자동입력", use_container_width=True, key="btn_user_rev"):
            st.session_state['app_running'] = False  # AI 가동 2차 차단
            
            with st.spinner("⏳ 신청인 사주간지 역산 중..."): 
                        ...
                        if gj_y and gj_y[0][:2] == ry_h:
                            ...
                            while curr_dt >= dt_mod.date(y, 1, 1):
                                ...
                                if len(gj) >= 3 and gj[0][:2] == ry_h and gj[1][:2] == rm_h and gj[2][:2] == rd_h:
                                    ...
                                    st.session_state['u_c'] = "양력" # 달력을 양력으로 강제 정렬
