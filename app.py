    ...
    # ---------------------------------------------------------
        ...
        with st.spinner("⏳ 두 분의 시공간을 교차 분석 중입니다..."):
            try:
                ...
                # 2. AI 통변 (상대방 데이터까지 포함된 데이터 시트 호출 확인 필요)
                ai_html = ""
                try:
                    # 박사님, AI에게 두 사람의 정보를 명확히 전달합니다.
                    prompt_content = f"신청인: {name}({birth_str}), 상대방: {f_name}({f_birth_str})의 궁합을 초연 시공명리 관점에서 분석하라."
                    ai_result = call_gemini_api(prompt_content)
                    if ai_result:
