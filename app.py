    ...
    # ---------------------------------------------------------
        ...
        with st.spinner("⏳ 두 분의 시공간을 교차 분석 중입니다..."):
            try:
                ...
                # 1. HTML 조각 생성 (박스 스타일 제외)
                birth_str = f"{b_year}년 {b_month}월 {b_day}일"
                f_birth_str = f"{f_y}년 {f_m}월 {f_d}일"
                
                cover_html = html_views.get_gunghap_cover(APP_VERSION, "♂️" if gender == "남성" else "♀️", name, gender, u_marital, birth_str, "♂️" if f_gender == "남성" else "♀️", f_name, f_gender, f_marital, f_birth_str, dt_mod.datetime.now().strftime("%Y년 %m월 %d일"))
                
                # 2. 내용 구성 (각 조각을 <div>로 감싸서 구조 유지)
                m_content = f"<div class='report-section'>{html_views.get_saju_table(*gh_data['m_table'])}{html_views.get_master_bar(*gh_data['m_master'])}</div>"
                w_content = f"<div class='report-section' style='page-break-before:always;'>{html_views.get_saju_table(*gh_data['w_table'])}{html_views.get_master_bar(*gh_data['w_master'])}</div>"
                closing = f"<div class='report-section'>{html_views.get_gunghap_closing(name, f_name)}</div>"
                ...
                try:
                    ...
                    ai_result = call_gemini_api(prompt_content)
                    if ai_result:
                        ai_html = f"<div class='report-section' style='margin-top:30px;'>{html_views.get_ai_report_box(ai_result.replace('###', ' ').replace('\n', '<p>'))}</div>"
                except Exception as e:
                    ai_html = f"<div style='color:red;'>AI 통변 오류: {e}</div>"

                # 4. 최종 결합
                # 표지(cover_html) 내부에 page-break가 포함되어 있으므로, 그 뒤에 content를 배치
                full_content = cover_html + m_content + w_content + ai_html + closing
