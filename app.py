...
<span style='font-weight: 900; font-size: 18px; color: #1A237E;'>- 초연 시공명리 연구소 드림 -</span>
                ...
                # 🎯 [최종 최적화: 한글 변환 없이 한자 키로 직접 매칭]
                # ms, mb, ds, db는 이미 원국 계산 로직에서 생성된 한자 변수입니다.
                w_key = f"{ms}{mb}" # 예: 丙寅
                i_key = f"{ds}{db}" # 예: 甲子
                
                # DB 탐색 (Key에 한자가 포함되어 있으므로 즉시 매칭)
                w_val = next((v for k, v in choyeon_db.get("wolryeong", {}).items() if w_key in k), "데이터 없음")
                i_val = next((v for k, v in choyeon_db.get("ilju", {}).items() if i_key in k), "데이터 없음")
