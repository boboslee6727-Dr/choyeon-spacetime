...
with st.sidebar:
    ...
    with st.expander("🔍 신청인 사주간지 역산", expanded=False):
        ...
        # on_change=stop_ai 추가: 글자 입력 시 AI 가동 방지
        with col_g1: ry = st.text_input("년주", value="", key="main_u_ry", on_change=stop_ai)
        with col_g2: rm = st.text_input("월주", value="", key="main_u_rm", on_change=stop_ai)
        with col_g3: rd = st.text_input("일주", value="", key="main_u_rd", on_change=stop_ai)
        with col_g4: rt = st.text_input("시주", value="", key="main_u_rt", on_change=stop_ai)
        
        if st.button("🔍 신청인 생년월일 자동입력", use_container_width=True, key="sb_btn_user_rev"):
            st.session_state['app_running'] = False  # AI 가동 차단
            
            with st.spinner("⏳ 신청인 사주간지 역산 중..."): # 명시적 스피너 추가
                        ...
                        if gj_y and gj_y[0][:2] == ry_h:
                            ...
                            while curr_dt >= dt_mod.date(y, 1, 1):
                                ...
                                if len(gj) >= 3 and gj[0][:2] == ry_h and gj[1][:2] == rm_h and gj[2][:2] == rd_h:
                                    ...
                                    st.session_state['sidebar_u_c'] = "양력" # 양력 강제 지정
    ...
    if "1. 개인사주" in u_product:
        ...
        with st.expander("👥 상대방 사주간지 역산", expanded=False):
            ...
            with p_col_g1: p_ry = st.text_input("년주", value="", key="sb_u_ry", on_change=stop_ai)
            with p_col_g2: p_rm = st.text_input("월주", value="", key="sb_u_rm", on_change=stop_ai)
            with p_col_g3: p_rd = st.text_input("일주", value="", key="sb_u_rd", on_change=stop_ai)
            with p_col_g4: p_rt = st.text_input("시주", value="", key="sb_u_rt", on_change=stop_ai)
            
            # [수정] 버튼 라벨 텍스트 변경 ("신청인" -> "상대방")
            if st.button("🔍 상대방 생년월일 자동입력", use_container_width=True, key="sb_btn_user_rev_p"):
                st.session_state['app_running'] = False  # 역산 시 AI 가동 차단
                
                with st.spinner("⏳ 상대방 사주간지 역산 중..."): # 명시적 스피너 추가
                            ...
                            if gj_y and gj_y[0][:2] == p_ry_h:
                                ...
                                while curr_dt >= dt_mod.date(y, 1, 1):
                                    ...
                                    if len(gj) >= 3 and gj[0][:2] == p_ry_h and gj[1][:2] == p_rm_h and gj[2][:2] == p_rd_h:
                                        ...
                                        st.session_state['f_c'] = "양력" # 상대방 양력 강제 지정
