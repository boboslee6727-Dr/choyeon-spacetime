...
"""
    ...
    """
            ...
            if u_product == "궁합":
                try:
                    ...
                    m_samjae_color = "#C62828" if m_samjae != "해당 없음" else "#555"
                    
                    m_ji_rel_rows = ""
                    for l_idx, r_idx in enumerate([1, 2, 0, 3]):
                        b_bot = "1px solid #444 !important" if l_idx == 3 else "0px solid transparent !important"
                        cells = "".join([f"<td style='color:{('#D50000' if ci==r_idx else ('#000' if get_ji_rel_set(m_jjis[r_idx], m_jjis[ci])!='-' else '#BBB'))}; font-weight:900; border-top:0px solid transparent !important; border-bottom:{b_bot}; border-left:1px solid #444 !important; border-right:1px solid #444 !important;'>{('←('+m_jjis[r_idx]+')→' if ci==r_idx else get_ji_rel_set(m_jjis[r_idx], m_jjis[ci]))}</td>" for ci in range(4)])
                        lbl = f"<td rowspan='4' class='header-cell-main' style='border-right: 1px solid #444 !important; border-left: 1px solid #444 !important; border-bottom: 1px solid #444 !important; border-top: 0px solid transparent !important; font-size:14px !important;'>합충형파해</td>" if l_idx==0 else ""
    ...
    """
                    ...
                    f_samjae_color = "#C62828" if f_samjae != "해당 없음" else "#555"
                    
                    f_ji_rel_rows = ""
                    for l_idx, r_idx in enumerate([1, 2, 0, 3]):
                        b_bot = "1px solid #444 !important" if l_idx == 3 else "0px solid transparent !important"
                        cells = "".join([f"<td style='color:{('#D50000' if ci==r_idx else ('#000' if get_ji_rel_set(f_jjis[r_idx], f_jjis[ci])!='-' else '#BBB'))}; font-weight:900; border-top:0px solid transparent !important; border-bottom:{b_bot}; border-left:1px solid #444 !important; border-right:1px solid #444 !important;'>{('←('+f_jjis[r_idx]+')→' if ci==r_idx else get_ji_rel_set(f_jjis[r_idx], f_jjis[ci]))}</td>" for ci in range(4)])
                        lbl = f"<td rowspan='4' class='header-cell-main' style='border-right: 1px solid #444 !important; border-left: 1px solid #444 !important; border-bottom: 1px solid #444 !important; border-top: 0px solid transparent !important; font-size:14px !important;'>합충형파해</td>" if l_idx==0 else ""
...
"""
                    ...
                    f_match = re.search(r'\[FEMALE_START\](.*?)\[FEMALE_END\]', clean_essay, re.DOTALL)
                    if f_match: female_part = f_match.group(1).strip()
                    ...
                    <div class='report-page'>
                        <div class='vip-inset-frame' style='border-color:#1B5E20; padding:20px;'>
                            ...
                            <div style='display:flex; justify-content:center; align-items:center; margin-bottom:20px;'>
                                <div style='width:130px; height:130px; border-radius:50%; background:conic-gradient({theme_color} {gh_engine.final_score}%, #f0f0f0 0); display:flex; justify-content:center; align-items:center; -webkit-print-color-adjust: exact;'>
                                    <div style='width:98px; height:98px; background:#fff; border-radius:50%; display:flex; flex-direction:column; justify-content:center; align-items:center;'>
                                        <span style='font-size:32px; font-weight:900; color:{theme_color};'>{gh_engine.final_score}</span>
                                        <span style='font-size:10px; color:#888; font-weight:bold;'>SCORE</span>
                                    </div>
                                </div>
                            </div>
                            <div style='text-align:center; margin-bottom:25px;'>
                                <span style='font-size:16px; font-weight:bold; color:#fff; background:{theme_color}; padding:8px 32px; border-radius:30px; display: inline-block; -webkit-print-color-adjust: exact;'>{gh_engine.grade}</span>
                            </div>
                            <div style='max-width:500px; margin:0 auto; margin-bottom:40px;'>{score_bars}</div>
                            {dynamic_closing}
                        </div>
                    </div>
                    """
