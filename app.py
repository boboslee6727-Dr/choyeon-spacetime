# ==============================================================================
# app.py (ver 81.0 Master - 수동 사이드바 100% 복구 및 스텔스 공장 통합본)
# ==============================================================================
...
# 💡 [영업부 호출]: URL을 가로채어 고객/관리자 접속을 통제합니다.
from pipeline_manager import run_pipeline_router
run_pipeline_router()

APP_VERSION = "ver 81.0 Master"
...
# ==============================================================================
# 🛡️ 1. [공장 자동화 스텔스 모드] 영업부 지시 시 사이드바를 숨기고 변수 자동 세팅
# ==============================================================================
if st.session_state.get('admin_proc_id'):
    ...
    # ==============================================================================
    # 🚨 2. [오리지널 수동 모드] 박사님의 연구용 사이드바 100% 완전 복구 🚨
    # ==============================================================================
    with st.sidebar:
        def stop_ai():
            st.session_state['app_running'] = False
...
if st.session_state.get('app_running', False):
    ...
    # 💡 [핵심 스텔스 UI] 관리자 모드일 때는 요란한 화면 대신 스텔스 메시지만 띄움
    is_admin_mode = st.session_state.get('admin_proc_id') is not None
    spinner_msg = f"⏳ [{name} 님의 감명서를 백그라운드에서 조용히 조립하고 있습니다...]" if is_admin_mode else f"⏳ [{u_product.strip()}] 시공명리 연산 및 정밀 통변 가동 중..."
    ...
    with st.spinner(spinner_msg):
            ...
            try:
                ...
                for i in range(10):
                    ...
                    p_daewun_data_list.append({
                        ...
                        "c_hanja": engine.K2H_GAN.get(p_c_hangul, p_c_hangul) if p_c_hangul != "-" else "-", "c_hangul": p_c_hangul, 
                        "j_hanja": engine.K2H_JI.get(p_j_hangul, p_j_hangul) if p_j_hangul != "-" else "-", "j_hangul": p_j_hangul,
                        "ss_ji": engine.get_ss(p_ds_hanja, p_j_hangul), "un_sung": engine.get_unsung(p_ds_hanja, engine.K2H_JI.get(p_j_hangul, p_j_hangul)),
                        "y_shinsal": engine.get_12_shinsal(p_yb, p_j_hangul) if p_j_hangul != "-" else "-", "d_shinsal": engine.get_12_shinsal(p_db, p_j_hangul) if p_j_hangul != "-" else "-", 
                        "is_current": (p_val <= p_age_val < p_val + 10), "is_first": (i == 0)
                    })
        ...
        if is_2person:
            ...
            if hasattr(html_views, 'get_couple_fact_split_layout'): part_1_fact_gunghap = html_views.get_couple_fact_split_layout(male_block, female_block)
            else: part_1_fact_gunghap = f"{male_block}<br>{female_block}"
        ...
        if hasattr(html_views, 'analyze_saju_facts_advanced'):
            ...
            adv_warning_str, health_erosion_str, action_solutions_str, spouse_issue_str = "정상 시공간 흐름", "특이 침식 파동 없음", "자연스러운 기운의 순환을 유지하며 긍정적 마음가짐 유지", "배우자궁 비교적 안정적 흐름 유지"

        adv_gan_data = {'year_gan': ys, 'month_gan': ms, 'day_gan': ds, 'hour_gan': hs}
        if hasattr(html_views, 'analyze_samja_combination'): samja_comb_facts = html_views.analyze_samja_combination(adv_gan_data, dw_g_cur)
        else: samja_comb_facts = "원국 특이 삼자조합 없음"
        
        try: shinsal_raw = engine.get_general_shinsal_filtered(1, gans, jjis, gender) if hasattr(engine, 'get_general_shinsal_filtered') else []
        except: shinsal_raw = []
...
- 사주팔자: 년주({male_data_pack[3]}), 월주({male_data_pack[2]}), 일주({male_data_pack[1]}), 시주({m_h_p})
- 격국: {m_gyuk_val}

[여명({f_name_val}) 사주 팩트]
- 명조: {f_sol_val}생 (음력 {f_lun_val}) / {f_time_val}
- 사주팔자: 년주({female_data_pack[3]}), 월주({female_data_pack[2]}), 일주({female_data_pack[1]}), 시주({f_h_p})
- 격국: {f_gyuk_val}
"""
        else:
            u_h_raw = f"{hs}{hb}"
...
"""
        ...
        cur_sewun_gan_val, cur_sewun_ji_val = engine.GAN[cur_sewun_base % 10], engine.JI[cur_sewun_base % 12]

        ilju_master_context = engine.get_ilju_master_prompt_context(f"{ds}{db}", choyeon_db)
        seun_first_half, seun_second_half = engine.get_seun_half_periods(target_year_val) if hasattr(engine, 'get_seun_half_periods') else ("상반기", "하반기")
        wolun_first_half, wolun_second_half = engine.get_wolun_half_periods(target_year_val, curr_m) if hasattr(engine, 'get_wolun_half_periods') else ("전반기", "후반기")

        user_entered_text = ""
        if u_product.startswith("4-1"): user_entered_text = (st.session_state.get("text_4_1", "")).strip()
        elif u_product.startswith("4-2"): user_entered_text = (st.session_state.get("text_4_2", "")).strip()
        if user_entered_text: user_entered_text = re.sub(r'[▷▶◈\[\]\■\□\●\○\◆\◇\★\☆\※\▪\▫]', '', user_entered_text)
        ...
        prompt_data = {
            ...
            "adv_warning_str": adv_warning_str, "health_erosion_facts": health_erosion_str,
            "samja_comb_facts": samja_comb_facts, "action_solutions": action_solutions_str, "spouse_issue_facts": spouse_issue_str,
            "dw_che": w_facts.get("dw_che", "대운 시공간 무대"), "ds": ds, "db": db, "gyukgook_detail": gyukgook_detail,
        ...
        try:
            ...
            if "1-1" in u_product:
                ...
                if hasattr(engine, 'get_weekly_calendar_data'): weekly_days_data = engine.get_weekly_calendar_data(selected_target_date, ds_hanja)
                else: weekly_days_data = []
                if hasattr(html_views, 'generate_weekly_calendar_html') and weekly_days_data: weekly_table_code = html_views.generate_weekly_calendar_html(weekly_days_data, selected_target_date.day, yb, db)
                else: weekly_table_code = "<div style='padding:15px; text-align:center; color:#C62828; font-weight:bold; background:#FFEBEE; border-radius:10px;'>🚨 주간운표 달력 생성 엔진 누락됨</div>"
                if "WEEKLY_CALENDAR_HERE" in ai_output_html: formatted_ai = sub_marker(ai_output_html, 'WEEKLY_CALENDAR_HERE', weekly_table_code)
                else: formatted_ai = f"{weekly_table_code}<br><br>{ai_output_html}"
                master_comp = f"{part_1_fact}{part_2_intro}{part_3_golden}{formatted_ai}{part_5_closing}"
                final_render_html = html_views.get_final_report_box(master_comp)
                ...
                if not user_entered_text:
                    ...
                    if hasattr(html_views, 'render_saju_comparison_report'): final_render_html = html_views.render_saju_comparison_report(part_1_fact, external_raw_box, full_ai_content)
                    else: final_render_html = html_views.render_comparison_report(part_1_fact, external_raw_box, full_ai_content)

            elif "3-1" in u_product:
                m_ess, f_ess, g_ess = "", "", clean_raw
                if gender == "남성": m_saju_html, f_saju_html = part_1_fact if 'part_1_fact' in locals() else "", p_part_1_fact
                else: m_saju_html, f_saju_html = p_part_1_fact, part_1_fact
                ...
                if hasattr(html_views, 'get_daewun_compare_box'): c_daewun_html = html_views.get_daewun_compare_box(m_name_val, m_daewun_html, f_name_val, f_daewun_html)
                else: c_daewun_html = f"<div>{m_daewun_html}<br>{f_daewun_html}</div>"
                ...
                if not user_entered_text:
                    ...
                    full_ai_content = golden_box_html + ("<br>" if golden_box_html else "") + formatted_ai
                    if hasattr(html_views, 'render_gunghap_comparison_report'): final_render_html = html_views.render_gunghap_comparison_report(part_1_fact_gunghap, external_raw_box, full_ai_content)
                    else: final_render_html = html_views.render_comparison_report(part_1_fact_gunghap, external_raw_box, full_ai_content)
        except Exception as e:
            final_render_html = f"<div style='color:red;'>🚨 렌더링 오류: {e}</div>"

        if 'final_render_html' not in locals() or final_render_html is None: final_render_html = ""
