    ...
    with st.sidebar:
        ...
        if main_category == "1. 개인 사주팔자 풀이 (종합)":
            u_product = st.radio("상세 분석 항목:", [
                ...
                "1-3. 이번 달 운세 상세분석", 
                "1-4. 주간 및 일운 상세분석"
            ], key="sub_category_1", on_change=stop_ai)
            ...
            u_product = st.radio("특성화 상품 선택:", [
                ...
                "2-6. 이사 택일", 
                "2-7. 개업 택일"
        ], key="sub_category_2", on_change=stop_ai)
        elif main_category == "3. 커플 연애/결혼운 (궁합) 풀이":
            u_product = st.radio("상세 분석 항목:", [
                "3-1. 결혼운 (궁합) 풀이", 
                "3-2. 결혼 택일", 
                "3-3. 출산 택일"
        ], key="sub_category_3", on_change=stop_ai)
        elif main_category == "4. 타 감명서 비교":
            u_product = st.radio("타 감명서 비교:", [
                "4-1. 타 감명서 비교 (사주)", 
                "4-2. 타 감명서 비교 (궁합)"
            ], key="sub_category_4", on_change=stop_ai)
