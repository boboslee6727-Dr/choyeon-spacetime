...
🚨 [궁성별 십성-십이운성 결합 통변 절대 규칙]: 
일간을 제외한 7궁위(년주, 월주, 일지, 시주)에 배치된 각 십성(十星)이 어떤 십이운성(十二運星)의 에너지 상태(예: 제왕, 건록, 목욕, 묘 등)에 놓여 있는지 그 역동성을 정밀하게 분석하십시오. 십성의 사회적 기능이 해당 궁성(년주=국가/선대, 월주=사회/부모, 일지=배우자/안방, 시주=자녀/미래) 및 나이대별 발현 시기와 맞물려 어떻게 질적으로 변화하고 체감되는지 현대 심리 상담의 구어체 에세이로 유기적으로 녹여내어 통변하십시오.)
...
(※ AI 지시: 제공된 12신살({s12_str})과 일반신살({shinsal_str}), 그리고 삼재 정보({cur_samjae}) 등을 유기적으로 분석하여 내담자가 가진 고유한 무기와 주의해야 할 타이밍을 설명하십시오. 🚨단, 전통 명리의 길흉화복이나 살(煞)의 공포를 조장하지 말고, 현대 심리상담 관점에서 부드러운 에세이로 풀어내십시오.)
</div>
