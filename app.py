...
"""
                    ...
                    # 8. 오리지널 프리미엄 VIP 표지 복원 및 세션 저장
                    from korean_lunar_calendar import KoreanLunarCalendar
                    from datetime import datetime

                    # 🚨 신청인(u) 데이터는 상단에 이미 존재함(sol_str, lun_str, u_age).
                    # 🚨 상대방(p) 역학 달력 데이터만 여기서 빠르게 단독 연산!
                    ...
                    p_leap_str = "윤달" if getattr(p_klc, 'isIntercalary', False) else "평달"
                    ...
                    p_age_val = curr_y - p_y + 1

                    # 성별에 따른 남명/여명 텍스트 대칭 배정
                    if u_gender == "남성":
                        m_n, m_a, m_s, m_l, m_t_val = u_name, u_age, sol_str, lun_str, u_t
                        f_n, f_a, f_s, f_l, f_t_val = p_name, p_age_val, p_sol_str, p_lun_str, p_t
                    else:
                        m_n, m_a, m_s, m_l, m_t_val = p_name, p_age_val, p_sol_str, p_lun_str, p_t
                        f_n, f_a, f_s, f_l, f_t_val = u_name, u_age, sol_str, lun_str, u_t

                    m_t_str = "" if m_t_val == "시간 모름" else f" {m_t_val}"
                    f_t_str = "" if f_t_val == "시간 모름" else f" {f_t_val}"

                    # 🚨 오리지널 디자인 100% 복원 (이름은 짙은 남색 강제 고정)
                    cover_html = f"""
                    <div class='report-page' style='display:flex; flex-direction:column; justify-content:center; align-items:center; height:100vh; background-color:#FAFAFA;'>
                        <div class='vip-inset-frame' style='border-color:#1B5E20; width:85%; padding:60px 40px; display:flex; flex-direction:column; justify-content:center; align-items:flex-start; box-shadow: 0 10px 30px rgba(0,0,0,0.1); background-color:#FFFFFF;'>
                            ...
                            <div style='width:100%; margin-bottom:30px;'>
                                <div style='font-size:20px; font-weight:bold; margin-bottom:10px;'>
                                    ♂️ <span style='color:#1A237E;'>{m_n}님</span> <span style='font-size:16px; color:#555;'>(남명, {m_a}세)</span>
                                </div>
                                <div style='font-size:15px; color:#333; font-weight:bold;'>
                                    [양력] {m_s} | [음력] {m_l}{m_t_str}
                                </div>
                            </div>

                            <div style='width:100%; margin-bottom:50px;'>
                                <div style='font-size:20px; font-weight:bold; margin-bottom:10px;'>
                                    ♀️ <span style='color:#1A237E;'>{f_n}님</span> <span style='font-size:16px; color:#555;'>(여명, {f_a}세)</span>
                                </div>
                                <div style='font-size:15px; color:#333; font-weight:bold;'>
                                    [양력] {f_s} | [음력] {f_l}{f_t_str}
                                </div>
                            </div>
                    ...
                    # 🚨 에러 완벽 척결: 46.2 버전에 없는 m_tbl, f_tbl을 지우고 곧바로 에세이 출력!
