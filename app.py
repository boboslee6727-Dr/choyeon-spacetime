...
with st.sidebar:
    ...
    with st.expander("🔍 신청인 사주간지 역산", expanded=False):
        ...
        if st.button("🔍 신청인 생년월일 자동입력", use_container_width=True, key="btn_user_rev"):
            ...
            # 💎 원본 71.0 방식 적용: 불필요한 특수문자만 제거 후 엔진 상수 직접 호출
    ...
    if u_product.startswith("1-"):
        ...
        # 🎯 [신규 추가] 전통명리 비교 체크박스
        is_compare_traditional = st.checkbox("⚖️ 전통명리 : 시공명리 운세풀이 비교)", value=st.session_state.get("is_compare_trad_val", False), key="is_compare_trad_val", help="체크 시 기존 시공명리 풀이 하단에 전통 명리학 단식 풀이와의 1:1 입체 비교 리포트가 추가 생성됩니다.")
...
if st.session_state.get('app_running', False):
    if any(x in u_product for x in ["1-", "3-1."]):
        ...
        with st.spinner(f"⏳ [{u_product.strip()}] 범용 시공명리 연산 및 정밀 분석 중...."):
            ...
            # 🛡️ 1. 만세력 변환 반환값(Unpacking) 오류 정석 방어
            try:
            ...
            # 🛡️ 2. 진기(절기) 연산 함수 존재 여부 체크 및 안전 우회
            ...
            day_wunseong, day_y_shinsal, day_d_shinsal = "건록", "망신살", "망신살"
            ...
            if "1-4." in u_product:
                ...
                for i in range(7):
                    ...
                    try:
                        g_r = engine.get_ganji_from_date(target_dt.year, target_dt.month, target_dt.day)
                        d_p = g_r[2] if len(g_r) > 2 else ("甲","子")
                    except Exception:
                        d_p = ("甲","子")
                    
                    ganji_str = f"{d_p[0]}{d_p[1]}" if len(d_p) >= 2 else "-"
                ...
                if hasattr(html_views, 'generate_weekly_daily_layout'):
            ...
            try:
                g_res_target = engine.get_ganji_from_date(curr_year, curr_m, curr_d)
                d_pillar_target = g_res_target[2] if len(g_res_target) > 2 else ("甲","子")
            ...
            try:
                t_res2 = engine.get_true_year_month_pillar(curr_year, curr_m, curr_d, 12, 0)
                m_p_target = t_res2[1] if len(t_res2) > 1 else ("乙", "未")
            ...
            if "3-1." in u_product or u_product == "타 감명서":
                try:
                    ...
                    if other_text_input and len(str(other_text_input).strip()) > 0:
                        ...
                        ) if hasattr(html_views, 'get_auto_comparison_cover') else ""
                        if other_cover_html: st.markdown(other_cover_html, unsafe_allow_html=True)
                        
                        report_2_html = html_views.get_other_report_original_html(other_text_input) if hasattr(html_views, 'get_other_report_original_html') else ""
                        if report_2_html: st.markdown(report_2_html, unsafe_allow_html=True)
                        
                        with st.spinner("⚖️ 1:1 상세 비교 리포트 분석 중..."):
                            u_name_val = name
                            u_gender_val = gender
                            u_age_val = age
                            u_marital_val = u_marital if 'u_marital' in locals() else "미혼"
        ...
        with st.spinner("💍 초연 시공명리학 기반으로 지정하신 날짜의 시공간 에너지를 심층 분석 중입니다..."):
            try:
                ...
                try:
                    g_res1 = engine.get_ganji_from_date(int(s_y), int(s_m), int(s_d))
                    m_db = g_res1[2][1] if len(g_res1)>2 and len(g_res1[2])>1 else "子"
                except: m_db = "子"
                
                try:
                    g_res2 = engine.get_ganji_from_date(int(p_y), int(p_m), int(p_d))
                    f_db = g_res2[2][1] if len(g_res2)>2 and len(g_res2[2])>1 else "丑"
                except: f_db = "丑"
                ...
                if date_mode == "기간 선택":
                    ...
                    try:
                        g_r = engine.get_ganji_from_date(target_date.year, target_date.month, target_date.day)
                        target_ganji = f"{g_r[2][0]}{g_r[2][1]}" if len(g_r)>2 else "알 수 없음"
                    except:
                        target_ganji = "알 수 없음"
