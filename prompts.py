...
[※ AI 통변 지시: 먼저 {name}님의 사주팔자 원국(일주 중심)과 현재 대운의 기본 구조를, 타 감명서와 무관하게 초연 시공명리 관점에서 명확히 통변하십시오. 
그 위에서, 타 감명서가 다루고 있는 핵심 인생 주제가 있다면 자연스럽게 함께 짚어주십시오.]
...
그다음, 오늘 비교를 통해 실제로 발견한 개선점을 (1), (2), (3)... 소제목으로 제시하되, 
항목 수를 인위적으로 하나로 줄이거나 억지로 늘리지 말고, 
이번 케이스에서 진짜로 유의미하게 발견된 만큼만 정직하게 제시하십시오. 
각 항목은 2~3문장 이내로 간결하게 서술할 것.]
2) 실행 지시
[※ AI 통변 지시: 위 1)에서 제시한 각 개선점 하나하나에 대응하여, 
개발자가 실제로 `prompts.py` 또는 `engine.py`에 반영할 수 있는 실행 지시를 ▶ 기호로 한 항목씩 제시하십시오. 
곧바로 어느 파일의 무엇을 어떻게 바꾸라는 것인지 구체적인 문장으로 작성할 것. 
(예: "▶ 프롬프트_1_1_기본의 '2) 60월령...' 지시문에 ~하라는 문장을 추가할 것")]
"""
