...
1) {dw_fact_str} 대운과 {curr_year} 년운의 상호작용
...
2. 상반기 {first_half_period} 운

1) 내 삶을 움직이는 주된 에너지와 환경
[※ AI 통변 지시: 폭포수 운세 팩트({woonse_fact_str})에 이미 계산되어 있는 상반기 키워드를 이번 해석의 절대적 핵심 방향으로 삼으십시오. 십성 명칭(재성·관성 등)만 보고 일반적인 통념(예: 재성=재물 걱정)으로 임의 재해석하는 것을 절대 금지합니다.
이 키워드를 바탕으로, 현재 대운의 큰 환경(體) 위에서 올해 상반기 세운 천간({cur_sewun_gan})이 실질적인 실행(用)으로 일으키는 에너지 방향성을 서술하십시오.
상반기가 '확장·도약기'인지 '수렴·내실기'인지는 이 키워드가 가리키는 방향에 맞추어 서술하고, 사회적·직업적 환경 변화를 알기 쉽게 설명하십시오.]
2) 현실적인 변화와 행동 가이드
[※ AI 통변 지시: 상반기 세운 천간이 원국과 맞물려 일으키는 시간방향 흐름과 합극 작용을 바탕으로, 
일·재물·인간관계에서 실제로 체감할 변화를 짚어주고 상반기를 유리하게 이끌 실전 행동 지침을 제시하십시오.]

3. 하반기 {second_half_period} 운

1) 흐름의 전환과 현실적 결실
[※ AI 통변 지시: 폭포수 운세 팩트({woonse_fact_str})에 이미 계산되어 있는 하반기 키워드를 이번 해석의 절대적 핵심 방향으로 삼으십시오. 십성 명칭만 보고 일반적인 통념으로 임의 재해석하는 것을 절대 금지합니다.
이 키워드를 바탕으로, 하반기 세운 지지({cur_sewun_ji})로 기운의 중심이 이동할 때 대운(體)과 맞물려 나타나는 실행(用)의 국면 전환을 서술하십시오.
상반기 에너지가 손에 잡히는 구체적 결과물로 안착하는 과정(확장 지속 또는 수렴 전환)을, 이 키워드가 가리키는 방향에 맞추어 일간의 시공간적 관점에서 명쾌하게 설명하십시오.]
