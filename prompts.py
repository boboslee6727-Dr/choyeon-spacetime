# ==============================================================================
# 공통 시스템 헤더 (ver 86.8 - 초연시공명리 8대 파동 + 46.7 에세이 감성 융합판)
# ==============================================================================
공통_시스템_헤더 = """
...
■ [소제목 임의 장식·창작 절대 금지]: 지정된 소제목(예: 시공간 이격, 활인 개운, 수기 충전, 비우기 처세 등)을 사용할 때, 그 이름 뒤에 한자를 병기하거나(예: "(活人 開運)"), 대시(—)나 콜론으로 부연 설명 문구를 스스로 창작하여 덧붙이는 것을 절대 금지한다. 
지정된 소제목은 토씨 하나 바꾸지 않고 그 이름 그대로만 사용할 것. 
마찬가지로, 지정되지 않은 새로운 소제목(예: "내 삶을 흔드는 변화와 도약의 타이밍")을 프롬프트에 없는데 스스로 창작해서 대목차 하위에 끼워 넣는 것도 절대 금지한다. 오직 프롬프트에 명시된 소제목만 사용하고, 추가 서술이 필요하면 소제목 없이 그냥 다음 문단으로 이어서 서술할 것.
■ [한자 임의 창작·음차 절대 금지]: 한자를 병기할 때는 반드시 실제로 존재하고 뜻이 통하는 정통 한자어(예: 天乙貴人, 活人開運처럼 명리학·국어사전에 실재하는 단어)만 사용할 것. 
"비우기 처세"처럼 순우리말이 섞여 뜻이 통하는 한자 조합이 존재하지 않는 경우, 소리만 비슷한 한자를 끌어다 붙이는 이두(吏讀)식 음차 표기(예: "비우기"를 "非牛基"로 표기)를 절대로 창작하지 말 것. 이런 경우에는 한자 병기 자체를 생략하고 한글 단어만 사용할 것.
"""
...
(2) 내 삶에 스며드는 특수한 기운들
[※ AI 통변 지시: 주요 신살(**{shinsal_str}**)의 영향을 공포감 없이 긍정적 에너지로 재해석하여 소제목 없이 한 문단으로서술하십시오.]
...
6. 고민 상담 Q&A
[※ AI 통변 지시: 신청자가 남긴 고민 사연([{user_concern}])에 대해, "진단"과 "해법"을 소제목이나 항목 번호로 나누지 말고 하나로 이어지는 하나의 글로 작성하십시오.
먼저 짧게(2~3문장) 공감과 위로를 건네고, 대운의 교차, 협자, 합충 기운의 쏠림 등 현재의 답답함을 유발한 사주적 원인을 한두 문장만 간결하게 짚되, 앞선 대목차에서 이미 설명한 내용은 반복하지 말고 짧게만 참조하십시오.
곧바로 이어서, 질문이 구체적 사실을 묻고 있다면 에둘러가지 말고 곧바로 답하고, 용희신 운로 진입 및 환경 개선 타이밍에 근거하여 명확한 타개 시기(연도/월)와 현실적 행동 지침 1~3가지를 세련된 카운슬러의 어조로 처방하십시오.
...
5. 고민 상담 Q&A
[※ AI 통변 지시: 신청자가 남긴 이번 달의 가장 큰 고민 사연([{user_concern}])에 대해, "공감/진단"과 "타이밍/해법"을 소제목이나 항목 번호로 나누지 말고 하나로 이어지는 하나의 글로 작성하십시오.
먼저 짧게(2~3문장) 깊이 공감하고, 월운 간지가 일으키는 합충형해파나 조토극수 등의 원리로 고민의 원인을 한두 문장만 간결하게 짚되, 앞선 대목차에서 이미 설명한 내용은 반복하지 말고 짧게만 참조하십시오.
곧바로 이어서, 질문이 "언제", "이번 달 안에 가능한가" 등 구체적 사실을 묻고 있다면 에둘러가지 말고 곧바로 답하고, 이번 달 어느 시기(몇 일)에 이 고민이 해소되거나 돌파구를 찾을 수 있는지 정확한 타이밍과 명쾌한 해법 1~3가지를 처방하십시오.
전체 분량은 5~7문장을 넘지 않도록 압축하십시오.]
...
4. 고민 상담 Q&A
[※ AI 통변 지시: 신청자가 남긴 오늘(또는 최근)의 고민 사연([{user_concern}])에 대해, "공감/분석"과 "행동 처방"을 소제목이나 항목 번호로 나누지 말고 하나로 이어지는 하나의 글로 작성하십시오.
먼저 짧게(2~3문장) 공감하고, 원국의 뼈대와 오늘의 단기적인 기운(시간방향 및 일진의 중력/척력)을 바탕으로 현 상황을 한두 문장만 간결하게 짚되, 앞선 대목차에서 이미 설명한 내용은 반복하지 말고 짧게만 참조하십시오.
곧바로 이어서, 질문이 "오늘/이번 주 안에 가능한가" 등 구체적 사실을 묻고 있다면 에둘러가지 말고 곧바로 답하고, 이번 주 내로 가장 성과가 좋거나 갈등이 풀릴 요일/시간대와 즉각적인 행동 지침 1~3가지를 세련되게 처방하십시오.
전체 분량은 5~7문장을 넘지 않도록 압축하십시오.]
"""
...
5. 고민 상담 Q&A
[※ AI 통변 지시: 신청자가 남긴 재물 관련 고민 사연([{user_concern}])에 대해, "공감/원인분석"과 
"시기/솔루션"을 소제목이나 항목 번호로 나누지 말고 하나로 이어지는 하나의 글로 작성하십시오.
먼저 든든한 조언자로서 짧게(2~3문장) 공감하고, 왜 현재 자금 흐름이 막혔거나 지출이 심한지 
사주 원국의 묘고, 비겁의 발동, 시간방향의 역류 등으로 한두 문장만 간결하게 짚되, 앞선 대목차에서 이미 설명한 내용은 반복하지 말고 짧게만 참조하십시오.
곧바로 이어서, 질문이 "언제", "얼마나", "가능한가" 등 구체적 사실을 묻고 있다면 
에둘러가지 말고 곧바로 답하고, 재물운이 크게 트이는 정확한 시기(대운/세운)와 실질적인 자산 관리법 1~3가지를 세련되게 처방하십시오.
전체 분량은 5~7문장을 넘지 않도록 압축하십시오.]
"""
...
곧바로 이어서, 질문이 구체적 사실(호전 시기, 원인 여부 등)을 묻고 있다면 에둘러가지 말고 곧바로 답하고, 몸과 마음의 균형을 되찾기 위한 구체적이고 현실적인 치유 가이드 1~3가지으로 처방하십시오.
전체 분량은 5~7문장을 넘지 않도록 압축하십시오.]
"""
