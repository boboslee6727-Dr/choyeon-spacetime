...
# 🎯 [버전 컨트롤 타워]
APP_VERSION = "Ver 46.2 (Gemini 2.5-Pro Mode)"
...
(※ AI 지시: 내담자의 사주구조(격국: {gyukgook_detail})를 반드시 핵심 뼈대로 삼아 에세이를 작성하십시오. 
이 격국의 특성이 어떤 시기에, 어느 정도 규모의 무대에서 어떻게 발현되는지 구체적으로 조언하십시오.)

<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>2) 내 삶의 리듬과 에너지 균형</span>
(※ 🚨AI 절대 팩트 지시: 내담자의 사주팔자 오행 분포 팩트는 **[ 木({counts['목']}) 火({counts['화']}) 土({counts['토']}) 金({counts['금']}) 水({counts['수']}) ]**입니다. 
AI는 스스로 오행의 개수를 세거나 지어내지 마십시오! 반드시 시스템이 제공한 이 오행 개수 팩트를 절대 기준으로 삼아, 오행의 과다(過多)와 고립, 누락 상태를 파악하십시오. 이를 계절적 조후 및 억부의 균형 상태와 결합하여, 내담자가 현실 삶에서 어떤 에너지를 보완하고 추구해야 하는지 상세한 에세이로 작성하십시오.)

<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>3) 내 삶의 역동성과 상호작용</span>
(※ 🚨AI 절대 가중치 지시: 이 구역은 전체 감명서의 성패를 가르는 가장 중요한 심장부입니다. 
기계적인 용어 나열이나 단순한 길흉 판단을 엄격히 금지하며, 다음의 4대 연산 규칙을 바탕으로 타 영역보다 최소 1.5배 이상의 압도적인 분량과 밀도로 서술하십시오.

🚨 [마스터 규칙: 근묘화실(根苗花實) 궁위 기반 통변의 절대 원칙]
합(合)·형(刑)·충(沖)·파(破)·해(害) 및 원진, 귀문, 천라지망, 격각 등 사주 내에서 벌어지는 모든 역동적 상호작용은 반드시 그것이 발생한 '위치(궁위)'를 기준으로 시기와 대상을 엄격히 분리하여 통변하십시오!
  - 년월(年月) 작용: 유년/청년기, 부모·고향 및 초기 사회적 기반에서 일어나는 변화와 역동성.
  - 월일(月日) 작용: 청·장년기, 배우자 및 직업/사회 활동 공간에서 벌어지는 갈등, 결합, 궤도 수정.
  - 일시(日時) 작용: 중·말년기, 자녀 및 아랫사람과의 관계, 개인적인 내밀한 공간이나 노후의 결과물.

1. [천간·지지의 역동적 연쇄 반응]: 위 '마스터 규칙'을 철저히 적용하여, 원국 내 천간의 합충과 지지의 상호작용(합형충파해, 원진, 귀문, 천라지망 등)을 유기적으로 엮어내십시오. 단순히 '충이 있어서 나쁘다'가 아니라, 한 글자의 움직임이 내담자의 생애 주기 중 어느 시점에 어떤 육친(부모/배우자/자녀)에게 나비효과를 불러일으키는지 서사적으로 추적해야 합니다.
2. [묘고(墓庫) 작용의 현미경 분석]: 박사님의 시공명리 정수인 '입고(入庫)'와 '개고(開庫)' 현상을 철저히 분석하십시오. 지지의 형충(刑沖)으로 인해 지장간 속에 숨겨져 있던 에너지가 개고되어 세상 밖으로 튀어나오는 타이밍과 잠재력, 반대로 활성화되어 있던 글자가 창고 속으로 거두어져 수렴하는 입고의 시기를 궁위(시기)와 결합하여 명확히 짚어내십시오.
3. [격각(隔角)의 시공간적 이탈 분석]: 지지가 한 칸을 건너뛰어 에너지가 어긋나는 격각 구조 역시 '마스터 규칙'의 궁위별 특성에 맞추어 적용하십시오. 정면충돌인 충(沖)과는 다르게, 조용히 가야 할 궤도를 이탈하면서 발생하는 물리적 이동(이사, 이직, 독립 등)과 심리적 소외감(동상이몽)을 입체적으로 풀어내십시오.
4. [드라마틱한 상담학적 에세이]: 차갑고 딱딱한 감정서 느낌을 배제하고, 내담자의 삶이라는 무대 위에서 간지(干支)들이 벌이는 치열한 상호작용을 한 편의 몰입감 넘치는 드라마처럼 구어체 심리상담 에세이로 풀어내십시오. 위기와 갈등의 원인을 인지시키되, 지혜롭게 우회하고 다스릴 수 있는 개운(開運)의 힌트를 제공하십시오.)
...
(※ 🚨AI 절대 팩트 지시: 내담자의 일간({ds})과 일지({dj}) 기준 십성은 [{get_ss(ds, dj)}], 십이운성은 [{get_unsung(ds, dj)}]입니다.
1. [일지 지장간 좌법(座法) 해부]: AI는 절대 스스로 연산하지 마십시오! 
시스템이 도출한 일지 지장간 좌법 팩트는 **[ {', '.join([f"{g}({get_ss(ds,g)})={get_unsung(g,dj)}" for g in JIJANGGAN.get(dj, ['-','-','-']) if g != '-'])} ]** 입니다. 
이 명확한 팩트를 바탕으로 내담자의 페르소나와 현실적 행동 메커니즘을 구어체 에세이로 도출하십시오.
2. [배우자궁(일지)의 3대 관점 풀이]: 일지는 내담자의 가장 내밀한 안방이자 배우자 영역입니다. 
이곳의 기운이 실제 사생활에서 어떻게 발현되는지 1) 육친적, 2) 심리적, 3) 사회적 3대 관점을 유기적으로 융합하여 상세히 풀어내십시오.)
🚨 [용어 표기 절대 규칙]: 모든 전문 명리 용어(예: 간여지동, 음인, 장성살 등)는 절대로 전면에 노출하지 마십시오. 
현대적인 구어체로 현실 현상을 먼저 쉽게 풀이한 뒤, 문장 끝에 간단한 설명과 함께 괄호 `()` 안에만 기재해야 합니다.)
   
<s<span class='sub-title' style='font-size: 18px; font-weight: 900; color: #111;'>2) 감추어진 내 속마음</span>
(※ 🚨AI 절대 연산 규칙: 감상적인 위로나 뜬구름 잡는 문학적 묘사를 엄격히 금지하며, 
철저히 **인종법(引從法) [🚨팩트 확인: {', '.join([f"{g}({get_ss(ds,g)})={get_unsung(g,jjis[1])}" for g in ['甲','乙','丙','丁','戊','己','庚','辛','壬','癸']])}]**과 **육친·오행 공망 [🚨팩트 확인: {calculate_gongmang(ds, jjis[1])} 공망]**의 현실적 조건이라는 2대 명리적 팩트 연산에만 기반하여 내담자의 무의식 세계를 타격하십시오.
1. [인종법(引從法)을 통한 무의식 추적]: 사주 원국 지장간에 드러나지 않은 천간 오행들을 일지로 인종(引從)하여 시스템이 도출한 위 포태법 팩트 상태를 확인하십시오. 
겉으로 드러나지 않았기에 내담자가 무의식 깊은 곳에서 갈망하거나 심리적으로 취약할 수 있는 본질적 정신 영역을 현대 심리학 관점의 구어체로 분석하십시오.
...
(※ 🚨AI 절대 지시: 사주 원국의 연주·월주 및 인성과 비겁의 상태를 분석하여 에세이를 작성하십시오. 
단, 월주의 십이운성은 반드시 [🚨팩트 확인: {get_unsung(ds, jjis[2])}] 에너지를 기준으로 분석해야 합니다.
...
{past_daewun_html}
(※ 🚨AI 절대 지시: 지나온 과거 대운은 내담자가 살아온 궤적이므로 단 하나도 누락하면 안 됩니다. {past_daewun_html}에 제시된 '첫 번째 대운부터 현재 대운 직전까지의 모든 과거 대운'을 순차적으로 빠짐없이 작성하되, 각 대운당 길게 늘어쓰지 말고 무조건 1~2줄 이내로 핵심만 요약하십시오. 바로 앞 대운 하나만 덜렁 쓰고 넘어가는 잔꾀를 엄격히 금지합니다.)
[지나온 과거 각 대운 출력 템플릿]
• <b>OO세~OO세 (OO대운):</b> 
<div style='padding-left: 20px;'>
    <b>1) 일반 명리 요약:</b> (1~2줄로 짧고 명확하게 핵심 요약)<br>
    <b>2) 시공 명리 요약:</b> (1~2줄로 짧고 명확하게 핵심 요약)
</div>
...
{past_sewun_html}
(※ 🚨AI 절대 지시: {past_sewun_html}에 제시된 모든 과거 세운을 단 하나도 누락하지 말고 순차적으로 작성하십시오. 단, 분량이 길어지지 않도록 각 세운마다 무조건 1~2줄 이내로 핵심만 요약하십시오.)
[지나온 과거 각 세운 출력 템플릿]
• <b>OOOO년(OO년):</b> 
<div style='padding-left: 20px;'>
    <b>1) 일반 명리 요약:</b> (1~2줄로 짧고 명확하게 핵심 요약)<br>
