...
def render_admin_panel(generator_func):
    ...
    # 💡 [진정한 정석 1단계]: DB 창고를 열고 'orders(장부)' 테이블이 존재하는지 메인 시스템에 정중히 묻습니다.
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='orders'")
    table_exists = cursor.fetchone()

    # 💡 [진정한 정석 2단계]: 장부가 있으면 당당하게 읽고, 서버 초기화로 날아갔으면 빈 장부를 새로 펼칩니다.
    if table_exists:
        df = pd.read_sql_query("SELECT * FROM orders ORDER BY created_at DESC", conn)
    else:
        df = pd.DataFrame() # 에러를 내지 않고 우아하게 빈 장부 생성
                        ...
                        if st.button(f"💰 입금 확인 (리포트 조용히 생성)", key=f"btn_pay_{row['order_id']}", use_container_width=True, type="primary"):
                            try:
                                with st.spinner(f"{row['name']}님의 정밀 분석 리포트를 생성 중입니다..."):
                                    ...
                                    st.session_state['s_t'] = row.get('b_time', row.get('birth_time', '시간 모름'))
                                    st.session_state['s_t_select'] = st.session_state['s_t']
                                    
                                    # 2. 👻 유령 주행 모드 On! (메인 공장 가동 스위치 켜기)
                                    st.session_state['app_running'] = True
                                    st.session_state['ghost_order_id'] = row['order_id']  # 납치할 주문번호 태그
                                    
                                    # 3. URL 파라미터를 비우고 메인 공장으로 강제 리로드!
    ...
    with tab2:
        ...
        if completed_orders.empty:
            ...
            for _, row in completed_orders.iterrows():
                # BASE_URL이 없으면 상대경로로 작동하도록 안전장치 추가
                v_url = f"{BASE_URL}/?mode=view&code={row['order_id']}" if 'BASE_URL' in globals() else f"/?mode=view&code={row['order_id']}"
                with st.expander(f"✅ [{row['name']} 님] (열람코드: {row['order_id']})", expanded=True):
                    st.write(f"- 연락처: **{row['phone']}** | [리포트 바로보기]({v_url})")
