# ==============================================================================
# pipeline_manager.py (ver 79.0 Master - 3단 서랍장 통합 상황실 및 마케팅 완자동화)
# ==============================================================================
...
def render_admin_panel():
    ...
    # 🔽 [서랍장 1] 영업 장부 및 대기열
    # =========================================================================
    with st.expander("📊 [서랍장 1] 영업 장부 및 대기열 (항상 열려있음)", expanded=True):
        ...
        if pending_orders.empty:
            ...
            for _, row in pending_orders.iterrows():
                ...
                if st.button(f"💰 입금 확인 (백그라운드 감명 시작) - {r_name}", key=f"pay_{r_oid}", type="primary"):
                    with st.spinner("공장(app.py)에 명식을 전달하고 감명서 생성을 시작합니다..."):
                        # 공장에 변수 세팅
                        st.session_state['u_n'] = r_name
                        st.session_state['u_g'] = row['gender']
                        st.session_state['u_m_stat'] = row['marital']
                        st.session_state['u_c'] = row['u_cal']
                        st.session_state['s_y'] = int(row['b_year'])
                        st.session_state['s_m'] = int(row['b_month'])
                        st.session_state['s_d'] = int(row['b_day'])
                        st.session_state['s_t'] = row['b_time']
                        st.session_state['s_t_select'] = row['b_time']
                        
                        if "3-" in engine_prod:
                            st.session_state['f_n'] = row['f_name']
                            st.session_state['f_g'] = row['f_gender']
...
# ------------------------------------------------------------------------------
# 🎯 [영업부 권한] 서랍장 2번(조련소) 및 3번(발송) - app.py가 결과물을 들고 여기로 옴
# ------------------------------------------------------------------------------
def render_admin_approval_ui(gid, final_render_html):
    ...
    # =========================================================================
    # 🔽 [서랍장 2] 감명서 미리보기 및 조련소
    # =========================================================================
    with st.expander("🔍 [서랍장 2] 감명서 미리보기 및 AI 조련소 (클릭 시 쫙 펼쳐짐)", expanded=True):
        st.components.v1.html(final_render_html, height=500, scrolling=True)
        st.markdown("---")
        st.markdown("#### 📝 AI 수정 지시사항 (선택)")
        st.caption("AI의 문투나 내용이 맘에 들지 않으면 아래에 지시사항(ex: '직업운 부분을 더 긍정적으로 써줘')을 적고 다시 돌려주세요. 직접 타자 치실 필요 없습니다.")
        feedback_text = st.text_input("지시사항 입력란", key=f"fb_{gid}", label_visibility="collapsed")
        
        if st.button("🔴 AI 다시 돌려! (지시사항 반영 재생성)", type="secondary", use_container_width=True):
            with st.spinner("박사님의 지시사항을 AI에 주입하여 전면 재생성 중입니다..."):
                st.session_state['ai_feedback_prompt'] = feedback_text
                st.session_state['app_running'] = True
                st.rerun()

    # =========================================================================
    # 🔽 [서랍장 3] 마케팅 멘트 통제소 및 발송
    # =========================================================================
    with st.expander("💌 [서랍장 3] 카톡 마케팅 문자 통제 및 최종 발송 (클릭 시 쫙 펼쳐짐)", expanded=True):
        ...
        if st.button("🟢 영업부 일 잘했네! 최종 발송 및 완료 처리", type="primary", use_container_width=True):
            with st.spinner("완벽한 마케팅 문자와 감명서를 발송 중입니다..."):
                save_report_to_db(gid, final_render_html)
                ...
                st.success(f"✅ [{gid}] 장부 저장 및 맞춤형 마케팅 발송 완료! 상단 서랍장 1번으로 돌아갑니다.")
                time.sleep(2)
                st.rerun()
